...
### 1. Redirection, and the rest of what real teams brought

**Decided and done: teams now come from the corpus.** 0024 found the pool all
self-play rested on was generated by a *singles* format. `champions_ai.data.
harvest` now assembles teams from the six Pokemon each replay names, with sets
pooled across the corpus and the engine as the only gate -- 752 of 783 pass
validation. Built from the **train split only**, because the corpus also
supplies the agreement benchmarks and 0014 already caught one split leak.

What that changed, with no code touched in the agent at all:
...
So the agent was never status-blind; it had no status moves. Corpus collection
is running toward 2,000 replays for set variety -- species coverage saturated
at 148 by 500, so this buys variety rather than breadth.

**The live item is what the new pool exposed.** Rage Powder is on 36% of
harvested teams and Follow Me on 9%, and **redirection is not modelled at
all** -- one passing comment in `legal_actions.py` and no implementation. The
agent uses Rage Powder (it falls through to the generic support value) and has
never once been measured against an opponent using it. It is the clearest
remaining case of the 0024 shape: a mechanic invisible because the pool hid it.

Before pricing it, measure what it is worth, the way 0023 should have been
approached: how often does a redirected attack change who faints?
