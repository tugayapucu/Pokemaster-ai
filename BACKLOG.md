...
### 1. Decide what teams we measure on

**Blocking, because it decides what every other number means.** Experiment
0024 found the pool all self-play rests on is generated by a *singles* format:
`gen9championsrandombattle` has no `gameType`, and Champions ships no random
doubles generator. Tailwind, Trick Room, Rage Powder, Follow Me and Helping
Hand are on **0.0%** of our teams and used by humans in 6-31% of real battles.
Protect: 89.4% of human battles, 22.0% of our teams. Swords Dance the other
way -- 57.5% of ours, 7.2% of theirs.

The engine-verified work is untouched (damage, turn order, mechanics), and so
are the agreement benchmarks, which already used real teams. What is on this
pool is every self-play win rate: 0020's exchange rate, 0021, 0022 and 0023.

Three ways out, and this is a decision rather than a task:

- **harvest teams from the corpus** -- `|poke|` gives species for all 500
  replays, and pooling observed sets per species across the corpus gives
  usage-weighted movesets. Partial by construction, and the closest thing to
  the real distribution we can get;
- **bias the generator toward doubles** -- cheaper, but it invents a
  distribution rather than measuring one;
- **accept and document** -- keep the pool, and stop treating self-play win
  rate as evidence about real play.

*(The previous item 1 -- Protect being underpriced -- is withdrawn. It rested
on comparing Protect's share of all human move choices with its share of all
agent move choices, which is meaningless across two different team
distributions. Measured against turns where Protect was actually available,
the agent uses it 10.0% of the time, and it is priced against the incoming
threat rather than flat. 0024 records both corrections.)*
