...
2. **`scout` the candidates** as they appear. Already built. *The first
   ...
   **The largest finding of the day: `scout` was mostly measuring the agent's
   ...
   | # | defect | where | kind |
   | --- | --- | --- | --- |
   | 1 | ~~**Charge moves** scored as immediate hits~~ — **done, 0050.** Priced at half a hit on the turn they charge, dropped from focus fire; a Mega's move now uses the weather its forme sets. Neutral (198/202), ships on correctness. **Extended in 0054:** recharge moves priced at 1 / (1 + accuracy) — a miss costs no recharge, read off `selfDrops` — and `matchup()` now prices both kinds for Team Preview and switching. Both A/Bs 200/200; E decided 0 of 200 matchups (uninformative), F 2 of 200 (neutral); both ship on correctness | `mechanics.charge`, `WEATHER_ON_ARRIVAL` | engine fact |
   ...
   | 3 | ~~Team Preview scores the **base forme**, not the Mega~~ — **done, 0052.** Ours: stone holders scored as their Megas, one per selection, the holder players evolve most often chosen when rates are loaded — 214/186, **ships**. Theirs: blended by measured Mega rate — 418/382 over 800, neutral, **off**. Still not modelled: a Mega's *ability* (`matchup()` reads no ability or item for either side), and one-Mega-per-battle on the opponent's side | `_team_preview_rows`, `_mega_assignments`, `build_mega_priors` | engine fact + a prior |
   | 4 | ~~Team Preview models **no field for its own side**~~ — **done, 0053, both parts ship.** `matchup()` now reads our own ability and item (207/193), and each candidate four is scored in the weather and terrain its own setters create (210/190). Simplifications: the field is up for the whole matchup, first setter of a kind wins. **Method note:** instrument checks had drawn the first twenty pool teams; 85% of the pool carries a setter, the first twenty all did, and a must-be-0 row went vacuous until re-run on a random forty (273 setter-less previews, 0 differed). Checks draw at random from now on | `_own_field`, `_best_team_preview`, `TERRAIN_ON_ARRIVAL`, `matchup(our_ability, our_item)` | fact about our team |
   | 5 | Team Preview rates each Pokemon **alone** — **investigated 2026-09-13; split into three.** (a) A data-driven *partner bonus* is not supportable: across 63 species–partner comparisons with 150+ games each way, 2 showed a detectable win-rate effect, about what chance gives. (b) **Speed beyond knockouts — closed for now.** 0055 (full credit): 385/415 over 800 battles, negative direction, off. 0056 swept partial credits 0.25 / 0.5 / 0.75 with 1.0 as a control on fresh seeds, pre-registered and pushed before any arm reported: 403/397, 388/412, 385/415, 389/411 — none clears even 95%, the control replicated 0055, and only the smallest credit is near even. Stays off; not revisited before Frankfurt. (c) Still open: support effects at Team Preview (Fake Out 76% of pool teams, Trick Room 56%, Intimidate 56%, Tailwind 55%, Helping Hand 39%, Follow Me 27%), and status moves, which `matchup()` ignores entirely | `order_edge`, `_score_selection` | judgement |
   ...
   - ~~**Mega Evolution lost every tie**~~ — **measured, 0060: neutral, so
     ...
     to the non-Mega; the recommender did the same on purpose. Players Mega on
...
- ~~Close the damage gap on M-C's new species~~ — **there is no gap.** 0046,
  after correction: 83.2% when a new species is involved against 80.8% when
  none is, intervals heavily overlapping. The first run reported a −9.2 point
  gap and was wrong: Mega formes were meant to be excluded, the exclusion tested
  the species *name*, and the lookup resolved by species — which is exactly what
...
**Four named gaps, raised 2026-09-10. Three closed the same day.**

| gap | state |
| --- | --- |
| Choice lock / Encore / Taunt / Disable | **closed** — `char locked <move>`, `char taunt`, `char disable <move>`, `char free` |
| PP not tracked | **closed** — `char used <move>`, `char pp <move> <n>`, seeded from the engine |
| turn number manual | **closed** — `n` advances the turn and ticks Tailwind, screens and Trick Room |
| M-C not simulatable | **blocked** on an npm build carrying `championsregmc`. Nothing in this repository can fix it |

Closing them turned up three things worth more than the gaps were:

- **Champions halves every protection move.** Protect has 8 PP here, not 16,
  and so do King's Shield, Spiky Shield, Baneful Bunker, Obstruct and Beak
  Blast. PP was judged low-value on the assumption a move cannot run out in a
  fifteen-turn game. For these, it can.
- **The corpus had quietly become two corpora** — 2,000 M-C replays against
  1,769 M-B, loaded together by `review`, for a regulation we cannot simulate.
  `load_all` now refuses a mixed corpus unless a format is named.
- **A test that saw nothing would have passed.** It read `move_id` off a
  `MoveAction`, which carries `move_index`. The baseline assertion caught it.
